...
CREATE TABLE h_biz_import_task (
    ...
    userId varchar(40) NULL,
    schemaCode varchar(120) NULL,
    taskStatus varchar(120) NULL,
    lastHeartTime timestamp NULL,
    threadName varchar(150) NULL,
    operationResultJson text NULL,
    originalFilename text NULL,
    CONSTRAINT h_biz_import_task_pkey PRIMARY KEY (id)
);

CREATE INDEX idx_h_b_i_t_userId ON h_biz_import_task USING btree (userId);
CREATE INDEX idx_h_b_i_t_schemaCode ON h_biz_import_task USING btree (schemaCode);
CREATE INDEX idx_h_b_i_t_threadName ON h_biz_import_task USING btree (threadName);
CREATE INDEX idx_h_b_i_t_heart ON h_biz_import_task USING btree (lastHeartTime,taskStatus);
